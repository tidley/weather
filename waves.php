<?php
// waves.php
// Server-side cached proxy for Open-Meteo Marine forecast.
...
$CACHE_FILE = __DIR__ . '/waves-cache.json';
...
$url = 'https://marine-api.open-meteo.com/v1/marine' . ($query ? ('?' . $query) : '');
...
$context = stream_context_create([
  'http' => [
    'method' => 'GET',
    'timeout' => 12,
    'header' => "User-Agent: waves-proxy/1.0\r\nAccept: application/json\r\n",
  ],
]);
...
if ($response === false) {
  ...
  header('X-Cache: MISS');
  send_json(json_encode(['error' => 'Failed to fetch marine upstream']), 502);
}
